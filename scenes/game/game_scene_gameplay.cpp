...
void GameScene::UpdateEnemies()
{
    ...
    // 3/19í«â¡ÅFÉãÅ[ÉvíÜÇ…m_entitiesÇïœçXÇµÇ»Ç¢ÇΩÇﬂÇÃàÍéûÉoÉbÉtÉ@(ìcîVè„èr)
    ...
    for (const auto& entity : m_entities)
    {
        ...
        // 3/19í«â¡ÅFÉAÅ[ÉLÉ^ÉCÉvï Ç…AIèàóùÇï™äÚ(ìcîVè„èr)
        if (enemy->GetArchetype() == EnemyArchetype::Walker)
        {
            ...
            constexpr float kWalkerDetectRange = 192.0f; // 4ÉOÉäÉbÉhÅ~48px 3/21èCê≥(ìcîVè„èr)
            constexpr float kWalkerAttackRange = 48.0f;
            constexpr float kWalkerDetectHeight = 96.0f; // 2ÉOÉäÉbÉhÅ~48px

            // 3/21èCê≥ÅFkWalkerDetectRangeÇ∆kWalkerDetectHeightÇégÇ§ÇÊÇ§èCê≥(ìcîVè„èr)
            ...
            case EnemyComponent::AIState::Attack:
                ...
                {
                    enemy->attackTimer = 0.0f;
                    // 3/21í«â¡ÅFçUåÇîÕàÕì‡Ç»ÇÁÉ_ÉÅÅ[ÉW(ìcîVè„èr)
        ...
        else if (enemy->GetArchetype() == EnemyArchetype::Ranged)
        {
            ...
            constexpr float kRangedDetectRange = 360.0f; // 15ÉOÉäÉbÉhÅ~48px 3/21í«â¡(ìcîVè„èr)
            constexpr float kRangedDetectHeight = 96.0f; // 2ÉOÉäÉbÉhÅ~48px 3/21í«â¡(ìcîVè„èr)

            enemy->attackTimer += m_lastDeltaTime;

            if (dist < kRangedDetectRange && enemy->attackTimer >= 3.0f) // CD3ïb 3/21í«â¡(ìcîVè„èr)
            {
                enemy->attackTimer = 0.0f;
                ...
                bullet->AddComponent<TransformComponent>(
                    transform->x + 24.0f,
                    transform->y + 24.0f,
                    48.0f, 24.0f); // â°1ÉOÉäÉbÉhÅ~èc0.5ÉOÉäÉbÉh 3/21èCê≥(ìcîVè„èr)
...
// 3/19í«â¡ÅFíeÇÃà⁄ìÆÅEìñÇΩÇËîªíËÅEçÌèú(ìcîVè„èr)
    ...
    m_entities.erase(
        std::remove_if(
            ...
            [&](const std::unique_ptr<Entity>& entity) -> bool
            {
                ...
                // 3/21í«â¡ÅFéıñΩÉ`ÉFÉbÉN(ìcîVè„èr)
                if (auto* lifetime = entity->GetComponent<PhotoCopyLifetimeComponent>())
                {
                    lifetime->Update(m_lastDeltaTime); // éËìÆçXêV
    ...
    m_entities.erase(
        std::remove_if(
            ...
            [](const std::unique_ptr<Entity>& entity)
            {
                ...
                // 3/21í«â¡ÅFMeleeAttackÇÃéıñΩÉ`ÉFÉbÉN(ìcîVè„èr)
