...
void GameScene::HandlePhotoCapture()
{
    ...
    m_photo.capture.items.clear();
    float capturedMaxRight = 0.0f;
    float capturedMaxBottom = 0.0f;
    CaptureEntitiesInFrame(frameX, frameY, frameWidth, frameHeight, capturedMaxRight, capturedMaxBottom);
    CaptureTilesInFrame(frameX, frameY, frameWidth, frameHeight, capturedMaxRight, capturedMaxBottom);
    if (m_photo.capture.items.empty())
    {
        return;
    }

    FinalizeCapturedPhoto(*player, capturedMaxRight, capturedMaxBottom);
}
...
void GameScene::HandlePhotoSpawn()
{
    ...
    if (!m_photo.capture.hasPhoto || !Input_IsKeyDown('E'))
    {
        return;
    }
    ...
    {
        m_photo.placement.layer = CyclePlacementLayer(m_photo.placement.layer);
    }
    if (Input_IsKeyPressed('F'))
    {
        m_photo.placement.flipX = !m_photo.placement.flipX;
    }
    if (Input_IsKeyPressed('B'))
    {
        m_photo.placement.bridgeEnabled = !m_photo.placement.bridgeEnabled;
    }
    ...
    if (!UpdatePhotoPlacementPreview(spawnX, spawnY, spawnWidth, spawnHeight))
    {
        return;
    }

    SpawnPhotoGroup(*player, spawnX, spawnY, spawnWidth);
}

void GameScene::CaptureEntitiesInFrame(float frameX, float frameY, float frameWidth, float frameHeight, float& capturedMaxRight, float& capturedMaxBottom)
{
    for (const auto& entity : m_entities)
    {
        ...
        item.appliedTheme = m_photo.capture.selectedTheme;
        ...
        ApplyPhotoFilterToCapturedTarget(*entity, m_photo.capture.selectedTheme);
        m_photo.capture.items.push_back(item);
...
void GameScene::CaptureTilesInFrame(float frameX, float frameY, float frameWidth, float frameHeight, float& capturedMaxRight, float& capturedMaxBottom)
{
    const float tileSize = m_tileMap.GetTileSize();
...
void GameScene::FinalizeCapturedPhoto(Entity& player, float capturedMaxRight, float capturedMaxBottom)
{
    m_photo.capture.hasPhoto = true;
    m_photo.capture.capturedTheme = m_photo.capture.selectedTheme;
    m_photo.placement.layer = PhotoCopyLayer::Foreground;
    m_photo.placement.flipX = false;
    m_photo.capture.width = (std::max)(1.0f, capturedMaxRight);
    m_photo.capture.height = (std::max)(1.0f, capturedMaxBottom);
    m_photo.capture.textureId = m_photo.capture.items.front().textureId;
    m_photo.capture.sourceX = m_photo.capture.items.front().sourceX;
    m_photo.capture.sourceY = m_photo.capture.items.front().sourceY;
    m_photo.capture.sourceWidth = m_photo.capture.items.front().sourceWidth;
    m_photo.capture.sourceHeight = m_photo.capture.items.front().sourceHeight;
    m_photo.capture.tintR = m_photo.capture.items.front().tintR;
    m_photo.capture.tintG = m_photo.capture.items.front().tintG;
    m_photo.capture.tintB = m_photo.capture.items.front().tintB;
    m_photo.capture.tintA = m_photo.capture.items.front().tintA;

    m_eventBus.Publish({ EventType::PlaySoundRequest, &player, nullptr, "scene_change", 0.0f, 0.0f });
    m_eventBus.Publish({ EventType::LogMessage, &player, nullptr, GetPhotoCaptureLogMessage(m_photo.capture.capturedTheme), 0.0f, 0.0f });
    m_shutterFlashRemaining = gShutterFlashSeconds;
}

bool GameScene::UpdatePhotoPlacementPreview(float& spawnX, float& spawnY, float& spawnWidth, float& spawnHeight)
{
    spawnWidth = std::max(32.0f, m_photo.capture.width);
    spawnHeight = std::max(32.0f, m_photo.capture.height);
    ...
    m_photo.placement.active = true;
    m_photo.placement.x = spawnX;
    m_photo.placement.y = spawnY;
    m_photo.placement.width = spawnWidth;
    m_photo.placement.height = spawnHeight;
    m_photo.placement.valid = IsPhotoPlacementValid(spawnX, spawnY, spawnWidth, spawnHeight);
    return m_photo.placement.valid && Input_IsMouseLeftPressed();
}

void GameScene::SpawnPhotoGroup(Entity& player, float spawnX, float spawnY, float spawnWidth)
{
    std::vector<CapturedPhotoItem> spawnedItems = m_photo.capture.items;
    ...
    AddBridgeSegments(spawnedItems, m_whiteTexture, m_photo.placement.flipX, m_photo.placement.bridgeEnabled, m_photo.capture.capturedTheme);

    if (m_photo.groups.activeGroupCount >= kMaxPhotoGroups)
    {
        const int groupToRemove = m_photo.groups.nextGroupId - m_photo.groups.activeGroupCount;
        m_entities.erase(
            std::remove_if(
                m_entities.begin(),
                m_entities.end(),
            ...
            m_entities.end());
        m_photo.groups.activeGroupCount = std::max(0, m_photo.groups.activeGroupCount - 1);
    }

    const int groupId = m_photo.groups.nextGroupId++;
    ...
    for (const auto& item : spawnedItems)
    {
        ...
        lastSpawnedBox->AddComponent<PhotoCopyLayerComponent>(
            item.layer == PhotoCopyLayer::Shadow ? PhotoCopyLayer::Shadow : m_photo.placement.layer);
    ...
    m_photo.groups.activeGroupCount = std::min(kMaxPhotoGroups, m_photo.groups.activeGroupCount + 1);
    m_photo.groups.hasSpawnedCopy = true;
