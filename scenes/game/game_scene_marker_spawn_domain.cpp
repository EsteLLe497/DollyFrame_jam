...
namespace
{
        ...
        for (int row = 0; row < tileMap.GetHeight(); ++row)
        {
            for (int column = 0; column < tileMap.GetWidth(); ++column)
            {
                ...
                if (marker == 'K')
                {
                    ...
                    // K2(J3 縺ｮ繧医≧縺ｫ謖・ｮ壹☆繧九→縲∝ｿ・ｦ∵焚2縺ｮ繧ｹ繧､繝・メ繧偵Μ繝ｳ繧ｯID 3縺ｸ謗･邯壹☆繧九・
        ...
        const std::vector<ElevatorMarker>& elevatorMarkers)
    {
        ...
        for (int elevatorIndex = 0; elevatorIndex < static_cast<int>(elevatorMarkers.size()); ++elevatorIndex)
        {
            ...
            // K4(Q 縺ｮ繧医≧縺ｪ謖・ｮ壹・縲∵怙蟇・ｊ縺ｮ蜷悟梛繧ｨ繝ｬ繝吶・繧ｿ繝ｼ縺ｸ謗･邯壹☆繧九・
    ...
    for (int row = 0; row < m_tileMap.GetHeight(); ++row)
    {
        for (int column = 0; column < m_tileMap.GetWidth(); ++column)
        {
            ...
            battery->AddComponent<BatteryComponent>(
                ...
                320.0f,
                1);
            // 髢句ｧ狗峩蠕後・螟ｧ縺阪↑deltaTime縺ｧ蠎翫ｒ謚懊￠縺ｪ縺・ｈ縺・・・鄂ｮ譎らせ縺ｧ謗･蝨ｰ繧堤｢ｺ螳壹☆繧九・
...
void GameScene::RefreshLinkedGimmicksFromMarkers()
{
    ...
    for (int index = 0; index < static_cast<int>(switchMarkers.size()); ++index)
    {
        ...
        {
            // 繧ｸ繧ｧ繝阪Ξ繝ｼ繧ｿ繝ｼ繧ゅせ繧､繝・メ縺ｨ蜷後§隗｣豎ｺ貂医∩繝ｪ繝ｳ繧ｯ繧貞茜逕ｨ縺吶ｋ縲・
...
void GameScene::RefleshSepiaRubblesFromMarkers()
{
    ...
    for (int row = 0; row < mapHeight; ++row)
    {
        for (int column = 0; column < mapWidth; ++column)
        {
            ...
            // 螟匁磁遏ｩ蠖｢繧ｵ繧､繧ｺ縺ｧTransform繧剃ｽ懊ｋ・医☆縺ｧ縺ｫ groupX/Y/Width/Height 縺ｯ險育ｮ玲ｸ医∩・・
            ...
            auto& groupComp = rubble->AddComponent<SepiaRubbleGroupComponent>(
                targetMarker,
                targetImageNo,          // startCell.imageNo 縺ｨ蜷後§
