...
namespace
{
    ...
    nlohmann::json BuildTuningJson()
    {
        ...
        root["camera_target_offset_x"] = gCameraTargetOffsetX;
        root["camera_target_offset_y"] = gCameraTargetOffsetY;
        root["camera_look_ahead_offset_x"] = gCameraLookAheadOffsetX;
        root["camera_look_ahead_response"] = gCameraLookAheadResponse;
        root["camera_look_ahead_return_response"] = gCameraLookAheadReturnResponse;
        root["camera_look_ahead_catch_up_speed_x"] = gCameraLookAheadCatchUpSpeedX;
        root["camera_dead_zone_y"] = gCameraDeadZoneY;
        root["camera_dead_zone_follow_speed_y"] = gCameraDeadZoneFollowSpeedY;
        root["camera_dead_zone_down_max_speed_y"] = gCameraDeadZoneDownMaxSpeedY;
        root["camera_dead_zone_strength_rise_response"] = gCameraDeadZoneStrengthRiseResponse;
        root["camera_dead_zone_strength_fall_response"] = gCameraDeadZoneStrengthFallResponse;
        root["camera_dead_zone_down_strength_response"] = gCameraDeadZoneDownStrengthResponse;
...
#undef UI_JSON_FIELD
    ...
    struct BackgroundPartPlacement
    {
        ...
        float width;
        float height;
        float parallax; // 1.0 = 繧ｫ繝｡繝ｩ縺ｨ螳悟・縺ｫ騾｣蜍・騾壼ｸｸ縺ｮ蝨ｰ蠖｢縺ｨ蜷後§)
    };

    // 莉ｮ驟咲ｽｮ縲ゅ≠縺ｨ縺ｧCSV繝槭・繧ｫ繝ｼ蛹悶☆繧句燕謠舌〒縲√％縺薙↓逶ｴ謗･蠎ｧ讓吶ｒ譖ｸ縺上・
...
namespace game_scene_detail
{
    ...
    void LoadTuningJsonFile()
    {
        ...
        gCameraTargetOffsetX = root.value("camera_target_offset_x", gCameraTargetOffsetX);
        gCameraTargetOffsetY = root.value("camera_target_offset_y", gCameraTargetOffsetY);
        gCameraLookAheadOffsetX = root.value("camera_look_ahead_offset_x", gCameraLookAheadOffsetX);
        gCameraLookAheadResponse = root.value("camera_look_ahead_response", gCameraLookAheadResponse);
        gCameraLookAheadReturnResponse = root.value("camera_look_ahead_return_response", gCameraLookAheadReturnResponse);
        gCameraLookAheadCatchUpSpeedX = root.value("camera_look_ahead_catch_up_speed_x", gCameraLookAheadCatchUpSpeedX);
        gCameraDeadZoneY = root.value("camera_dead_zone_y", gCameraDeadZoneY);
        gCameraDeadZoneFollowSpeedY = root.value("camera_dead_zone_follow_speed_y", gCameraDeadZoneFollowSpeedY);
        gCameraDeadZoneDownMaxSpeedY = root.value("camera_dead_zone_down_max_speed_y", gCameraDeadZoneDownMaxSpeedY);
        gCameraDeadZoneStrengthRiseResponse = root.value("camera_dead_zone_strength_rise_response", gCameraDeadZoneStrengthRiseResponse);
        gCameraDeadZoneStrengthFallResponse = root.value("camera_dead_zone_strength_fall_response", gCameraDeadZoneStrengthFallResponse);
        gCameraDeadZoneDownStrengthResponse = root.value("camera_dead_zone_down_strength_response", gCameraDeadZoneDownStrengthResponse);
...
namespace
{
    ...
    PhotoState DeserializePhotoState(const json& root)
    {
        ...
        // 遐ｴ謳阪・謇狗ｷｨ髮・そ繝ｼ繝門ｯｾ遲・ 繧ｹ繝ｭ繝・ヨ逡ｪ蜿ｷ縺ｯ蠢・★驟榊・遽・峇縺ｫ蜿弱ａ繧九・
        // 遽・峇螟厄ｼ育音縺ｫ雋蛟､・峨′蜈･繧九→蜀咏悄繝医Ξ繧､縺ｮ蜑ｰ菴呵ｨ育ｮ励〒雋繧､繝ｳ繝・ャ繧ｯ繧ｹ縺ｫ縺ｪ繧顔ｯ・峇螟悶い繧ｯ繧ｻ繧ｹ縺吶ｋ縲・
...
void GameScene::ResetSceneState()
{
    ...
    // 繝懊せ謌ｦ繧ｫ繝｡繝ｩ縺ｮ蟾ｦ蜿ｳ蜿崎ｻ｢迥ｶ諷九ｒ縲・壼ｸｸ縺ｮ蜿ｳ蛛ｴ讒句峙縺ｸ謌ｻ縺励∪縺吶・
...
bool GameScene::LoadProgressStateFromDisk()
{
    ...
    // 繝√ぉ繝・け繝昴う繝ｳ繝医・襍ｷ蜍穂ｸｭ縺縺第怏蜉ｹ縺ｨ縺励・℃蜴ｻ繝舌・繧ｸ繝ｧ繝ｳ縺ｮ菫晏ｭ伜､縺ｯ蠕ｩ蜈・＠縺ｪ縺・・
    ...
    {
        // 譌ｧ蠖｢蠑上・螳御ｺ・ヵ繝ｩ繧ｰ繧偵メ繝･繝ｼ繝医Μ繧｢繝ｫ1逡ｪ縺ｸ遘ｻ陦後＠縺ｾ縺吶・
    ...
    {
        // 蜿､縺・メ繧ｧ繝・け繝昴う繝ｳ繝亥慍轤ｹ縺九ｉ蜊ｳ蠎ｧ縺ｫ蜀崎ｵｷ蜍輔＠縺ｪ縺・ｈ縺・・幕蟋句慍轤ｹ縺ｸ遘ｻ陦後☆繧九・
...
bool GameScene::SaveProgressState()
{
    ...
    // 繝√ぉ繝・け繝昴う繝ｳ繝医・繧ｲ繝ｼ繝邨ゆｺ・凾縺ｫ遐ｴ譽・＠縲∵ｬ｡蝗槭・繧ｹ繝・・繧ｸ髢句ｧ句慍轤ｹ繧剃ｽｿ縺・・
    m_save.hasCheckpoint = false;
    m_save.activeCheckpointId = -1;
...
void GameScene::InitializeStageResources(ResourceManager& resources)
{
    ...
    // 謦ｮ蠖ｱ繝ｻ蜀咏悄繝医Ξ繧､縺ｧ譛蛻昴↓菴ｿ縺・ユ繧ｯ繧ｹ繝√Ε繧貞・隱ｭ縺ｿ縺吶ｋ縲・
    // 驕・ｻｶ繝ｭ繝ｼ繝峨・縺ｾ縺ｾ縺縺ｨ蛻晏屓繧ｷ繝｣繝・ち繝ｼ縺ｮ繝輔Ξ繝ｼ繝縺ｧ繝・ぅ繧ｹ繧ｯI/O縺瑚ｵｰ繧翫√き繧ｯ縺､縺阪↓縺ｪ繧九・
...
void GameScene::InitializeStageEntities()
{
    ...
    for (const TileMarker& stageMarker : stageMarkers)
    {
        ...
        else if (marker == 'A') // 郢ｧ・ｴ郢晢ｽｼ郢ｧ・ｹ郢昴・
        {
            Entity& enemy = SpawnStagePrefab(
                prefabs,
                "sandbox_enemy_ghost",
        ...
        else if (marker == 'D') // 郢晄じﾎ帷ｹ晢ｽｭ郢昴・
        {
            ...
            if (auto* transform = enemy.GetComponent<TransformComponent>())
            {
                // FindSpawnPosition郢ｧ蜑・ｽｽ・ｿ郢ｧ荳岩・CSV邵ｺ・ｮ陟趣ｽｧ隶灘生・堤ｸｺ譏ｴ繝ｻ邵ｺ・ｾ邵ｺ・ｾ闖ｴ・ｿ邵ｺ繝ｻ
                ...
                {
                    // 陞滂ｽｩ闔蜍滓・陞ｳ螟ｲ・ｼ螢ｹ繝ｻ郢晢ｽｼ郢ｧ・ｫ郢晢ｽｼ邵ｺ・ｮ闕ｳ鄙ｫ繝ｻ郢ｧ・ｿ郢ｧ・､郢晢ｽｫ邵ｺ謔滂ｽ｣竏壺・郢ｧ迚呻ｽ､・ｩ闔謌奇ｽｨ・ｭ驗ゑｽｮ
    ...
    // 蜷後§蛻励↓隍・焚縺ゅｋ蝣ｴ蜷医・縲∝慍髱｢縺ｫ霑代＞荳逡ｪ荳九・繝槭・繧ｫ繝ｼ繧貞ｾｩ蟶ｰ蝨ｰ轤ｹ縺ｨ縺励※謗｡逕ｨ縺吶ｋ縲・
